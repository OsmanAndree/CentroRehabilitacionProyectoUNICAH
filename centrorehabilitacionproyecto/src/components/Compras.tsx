...
import { fetchCompras, deleteCompra } from '../features/compras/comprasSlice';
...
function Compras() {
  ...
  const eliminarCompraHandler = (id_compra: number) => {
    if (!window.confirm("¿Está seguro que desea inactivar esta compra?")) return;
    dispatch(deleteCompra(id_compra))
      .unwrap()
      .then(() => toast.success("Compra inactivada exitosamente"))
      .catch((err) => toast.error(`Error al inactivar: ${err.message}`));
  };
    ...
    <Container fluid className="px-3 px-sm-4 px-md-5 py-4">
      ...
      <Card className="shadow-lg border-0" style={{ borderRadius: "20px", backgroundColor: "#ffffff" }}>
        ...
        <Card.Body className="p-3 p-md-4">
          ...
          {status === 'loading' ? (
            <div className="text-center py-5"><Spinner animation="border" variant="success" /><p className="mt-3 text-muted">Cargando compras...</p></div>
          ) : status === 'failed' ? (
              ...
              <Table hover className="align-middle mb-0">
                ...
                <tbody>
                  {comprasFiltradas.length > 0 ? (
                        ...
                        <td className="py-3 px-4 text-center">
                          <div className={`d-flex ${isMobile ? 'flex-column' : 'justify-content-center'}`} style={{ gap: isMobile ? '8px' : '6px' }}>
                            ...
                            <PDFDownloadLink
                                ...
                                className={`btn btn-outline-success btn-sm ${isMobile ? 'w-100' : ''}`}
                                style={{ borderRadius: "8px", padding: "0.4rem 0.6rem" }}
                            >
                                {({ loading }) => (
                                    <span>
                                        <FaFileInvoice className={isMobile ? "me-2" : ""} /> {loading ? "..." : (isMobile ? "Factura" : "")}
                                    </span>
                                )}
                            </PDFDownloadLink>
                          </div>
                    ...
                    <tr><td colSpan={5} className="text-center py-5 text-muted">No se encontraron compras.</td></tr>
                  )}
                </tbody>
              </Table>
            </div>
